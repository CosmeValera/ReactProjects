...
In the **Refresh token call** we used the param **refresh_token** fom the **Login CALL**.

**Confidential Client Call:**

![alt text](image/image-35.png)

As you can see, the **Confidential Client Call** only returns an **access_token** and no **refresh_token**. This is because **refresh_tokens** exist to avoid repeatedly asking the user for their password. However, in confidential client calls, since it's server to server communication, there's no user password involved. The client can always authenticate directly with its credentials, so there's no need for a refresh token. The access_token you obtain is all you need for server to server authentication.
